...
func main() {
	...
	logger.CHECK(logger.BroadcastLogNow(logger.GenerateRegistrationMsg(globalNodeID, "origin-server")))

	log.Println("Generating random strings")
	logger.CHECK(logger.BroadcastLog(logger.GenerateInfoLog(globalNodeID, "origin-server", "Generating random strings")))
	...
	logger.CHECK(logger.BroadcastLog(logger.GenerateInfoLog(globalNodeID, "origin-server ", "Random strings generated")))

	pc, err := net.ListenPacket("udp", ":7777") // Listen on port 7777
	log.Println("Listening on port 7777")
	logger.CHECK(logger.BroadcastLog(logger.GenerateInfoLog(globalNodeID, "origin-server", "Listening on port 7777")))
	...
	for {
		...
		if err != nil {
			...
			logger.CHECK(logger.BroadcastLogNow(logger.GenerateWarnLog(globalNodeID, "origin-server", "Error reading from UDP")))
			continue
		}
		log.Printf("Received message from %s: %s\n", addr, string(buffer[:n]))
		logger.CHECK(logger.BroadcastLog(logger.GenerateInfoLog(globalNodeID, "origin-server", "Received message from "+addr.String()+": "+string(buffer[:n]))))
		...
		if err != nil {
			...
			logger.CHECK(logger.BroadcastLogNow(logger.GenerateWarnLog(globalNodeID, "origin-server", "Error converting key to int")))
			continue
		}
		value, ok := dictionary[key] // Get the value from the dictionary
		if !ok {
			log.Println("Key not found")
			logger.CHECK(logger.BroadcastLogNow(logger.GenerateWarnLog(globalNodeID, "origin-server", "Key not found")))
			continue
		}
		log.Println("Sending value to client", value)
		logger.CHECK(logger.BroadcastLog(logger.GenerateInfoLog(globalNodeID, "origin-server", "Sending value to client "+value)))
		_, err = pc.WriteTo([]byte(value), addr) // Send the value to the client
		if err != nil {
			log.Println("Error writing to UDP:", err)
			logger.CHECK(logger.BroadcastLogNow(logger.GenerateWarnLog(globalNodeID, "origin-server", "Error writing to UDP")))
		}
	}
}
